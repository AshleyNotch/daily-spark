...
function deadlineUrgent(d?: string | null, status?: string) {
  ...
  const hrs = differenceInHours(parseISO(d), new Date());
  return hrs <= 48;
}
...
function DashboardPage() {
  ...
  const load = async () => {
    ...
    setBrands(b ?? []);
    const map = new Map((b ?? []).map((x) => [x.id, x] as const));
  ...
  return (
    <div className="px-6 lg:px-10 py-6 max-w-[1400px] mx-auto">
      ...
      {hasSession === false && (
        <button
          onClick={() => toast("AI check-in coming soon")}
      ...
      {brandFilter && (
        <div className="mt-4 text-sm">
          Filtered by brand —{" "}
          <Link to="/dashboard" className="underline">clear</Link>
        </div>
      )}
  ...
  return (
    <div className="rounded-xl border border-dashed border-border py-16 px-6 text-center">
      ...
      <p className="text-sm text-muted-foreground mt-1">
        Start your morning check-in or add a task manually.
      </p>
...
}: { tasks: Task[]; onEdit: (t: Task) => void; onChanged: () => void }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      {grouped.map(([prio, items]) => (
        <div key={prio}>
          <div className="px-5 py-1.5 text-[11px] uppercase tracking-wider text-muted-foreground bg-muted/20">
            Priority {prio}
          </div>
          {items.map((t) => (
            <button
